...
use db_SQLCaseStudies
  ...
  --Count of rows
  SELECT 
    (SELECT COUNT(*) FROM DIM_CUSTOMER) as 'Customer Count',
    (SELECT COUNT(*) FROM DIM_DATE) as 'Date Count',
    (SELECT COUNT(*) FROM DIM_LOCATION) as 'Location Count',
    (SELECT COUNT(*) FROM DIM_MANUFACTURER) as 'Manufacturer Count',
    (SELECT COUNT(*) FROM DIM_MODEL) as 'Model Count',
    (SELECT COUNT(*) FROM FACT_TRANSACTIONS) as 'Transaction Count'
...
;WITH CustomerSpend AS (
    SELECT customer_name, YEAR(date) as year, SUM(totalprice) as totalprice, AVG(Quantity) as avg_qty
    FROM DIM_CUSTOMER as t1
    JOIN FACT_TRANSACTIONS as t2
    ON t1.IDCustomer=t2.IDCustomer
    GROUP BY customer_name, YEAR(date)
), RankedCustomers AS (
    SELECT customer_name, year, totalprice, avg_qty,
           ROW_NUMBER() OVER(PARTITION BY year ORDER BY totalprice DESC) as row_number
    FROM CustomerSpend
)

SELECT customer_name, year, AVG(totalprice) as avg_price, AVG(avg_qty) as avg_qty,
